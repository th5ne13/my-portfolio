...
public class RnoticeViewAction implements Action {
	public ActionForward execute(HttpServletRequest request, 
		...
		RnoticeInfo rnoticeInfo = rnoticeService.getNotice(num);
		// 글번호에 해당하는 글의 데이터를 NoticeInfo형 인스턴스로 받아옴(글수정을위해서)

		if (rnoticeInfo == null) {	// 게시글 정보가 없으면
		...
		request.setAttribute("rnoticeInfo", rnoticeInfo);
		// 필요한 데이터들은 request객체의 속성으로 담아서 가져감
		// 단, Dispatcher방식일 경우에만 사용가능

		ActionForward forward = new ActionForward();
		forward.setPath("/list.notice");
