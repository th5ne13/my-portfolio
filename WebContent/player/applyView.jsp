...
String uniqueRevNum = "1" + mlid + num + "A";
...
function chkFun(all) {
	...
	if (<%=testval %> > 0) {
		if (textpwd == (<%=realpwd %>)){
			...
			all.action="#"; // 여기까진 먹음 
			opener.location.href="up.anotice?num=<%=num %>&realpwd=<%=realpwd %>&upid=<%=upid %>&title=<%=title %>&name=<%=name %>&addr=<%=addr %>&plan=<%=plan %>&phone=<%=phone %>&isend2=<%=isend2 %>&level=<%=level %>&position=<%=position %>";
			self.close();
			return true;
		} else {
...
<form name="detail" onsubmit="return chkFun(this);" action="recruit.request" method="post" >
	...
	<input type="hidden" name="reqtype" value="A" />
